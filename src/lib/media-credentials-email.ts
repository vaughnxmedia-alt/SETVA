...
export { compileMediaCredentialApprovalConfirmationEmail } from "@/lib/media-credential-approval-email";
...
export async function sendMediaCredentialApprovalConfirmationEmail(
  ...
  const compiled = compileMediaCredentialApprovalConfirmationEmail({
    ...
    checkInLocation: options.checkInLocation,
  });

  const resend = resendClient();
  if (!resend) {
    console.info("[demo] Media credential approval confirmation:", application.email, {
      checkInTime: compiled.checkInTime,
      checkInLocation: compiled.checkInLocation,
    });
    return;
  }
